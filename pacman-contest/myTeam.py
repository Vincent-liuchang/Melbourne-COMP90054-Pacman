...
class myAgent(CaptureAgent):
    ...
    def getFeatures(self, gameState, action):
        ...
        if self.getEnermy(successor)[0][2] - e1[2] > 1 and successor_pos[0] != self.start[0] and successor_pos[1] != self.start[1]:
            features["ghostMin"] = -1000
            return util.Counter(features)
        if self.getEnermy(successor)[1][2] - e2[2] > 1 and successor_pos[0] != self.start[0] and successor_pos[1] != self.start[1]:
        ...
        if self.goBackScore(gameState, successor):
            features["foodCarry"] = abs(successor.getAgentState(self.index).numCarrying-gameState.getAgentState(self.index).numCarrying) * 10000
        ...
        if not self.foodSafe() or (self.foodSafe() and gameState.getAgentState((self.index+2) % 4).isPacman)\
                or (self.myDistance[self.start] > 10000 and min(self.getCapsuleDistance(gameState)) > 10000) or min(self.getCapsuleDistance(gameState)) < 10000:
            if (self.getEnermyDistanceToMe(gameState)[0] < 5 and gameState.getAgentState(self.index).isPacman and (not self.getEnermy(gameState)[0][3]) and (not self.getEnermy(gameState)[0][0]))\
                    or (self.getEnermyDistanceToMe(gameState)[1] < 5 and gameState.getAgentState(self.index).isPacman and (not self.getEnermy(gameState)[1][3]) and (not self.getEnermy(gameState)[1][0]))\
                ...
                if (self.foodLeft <= 2 and self.getHomeDistance(gameState) < 10000) or len(self.getCapsules(gameState)) == 0 or min(self.getCapsuleDistance(gameState)) > 10000:
                    features["friend"] = self.getTeamateDistance(gameState) - self.getTeamateDistance(successor)
        ...
        if self.goEat(gameState, successor):
            ...
            if min(self.getCapsuleDistance(gameState)) == 99999 and self.getTargetFoodDistance(gameState) == 99999 and min(self.getEnermyDistanceToMe(gameState)) <= 2:
                    features["distanceHome"] = self.getHomeDistance(gameState) - self.getHomeDistance(successor)
            if (not gameState.getAgentState(self.index).isPacman) and self.getEnermyDistanceToMe(gameState)[0] <= 3 and (not e1[0]) and (not e1[3]) and (e1[1] is not None):
                print("wondering")
                if len(self.food) != 0:
                    self.targetFood = random.choice(self.food)
                features["food"] = e1[2]/10
                features["capsule"] = 0
            if (not gameState.getAgentState(self.index).isPacman) and self.getEnermyDistanceToMe(gameState)[1] <= 3 and e2[2] < e1[2] and (not e2[0]) and (not e2[3])  and (e2[1] is not None):
                print("wondering")
                if len(self.food) != 0:
                    self.targetFood = random.choice(self.food)
    ...
    def goBackScore(self, gamestate, successor):
        if gamestate.getAgentState(self.index).numCarrying - successor.getAgentState(self.index).numCarrying > 1 \
            ...
            return True
        if self.foodLeft <= 2 and gamestate.getAgentState(self.index).numCarrying - successor.getAgentState(self.index).numCarrying == 1 \
                and not successor.getAgentPosition(self.index) == successor.getInitialAgentPosition(self.index):
            return True
        return False
            ...
            if max(Qvalue) == 0:
                ...
                if self.foodLeft <= 2:
                    if "West" in good_action:
                        ...
                        action = np.random.choice(good_action)
                else:
                    action = np.random.choice(good_action)
        else:   # 随机选择 action
