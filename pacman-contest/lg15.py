...
# no capsule map
# one stuck two
# zuihou mubiao queding yihou huifang
# hai shi bu gou kuai
# an quan shi wu
# bianxian song
# bei kashiyeyou duiyouhuijia
# zai yuan duan faxian diren
#################
# Team creation #
#################
position1 = 1
...
class MonteCarloOffensiveAgent(CaptureAgent):
    ...
    def chooseFood(self, foodList):
        ...
        if position1 == 1:
            ...
            if self.startState.isOnRedTeam(self.index):
    ...
    def chooseAction(self, gameState):
        ...
        if (self.food in self.foodDefend and self.position != self.food) and gameState.getAgentState(self.index).scaredTimer == 0:
            print()
            print(self.food)
            print(self.position)
            pass
        elif self.food not in self.getFood(gameState).asList():
            if foodLeft > 2:
    ...
    def getReward(self, gameState):
        ...
        if len(foodList) > 0:
            reward[self.nearestFood(foodList)] = 1

        peerIndex = (self.index + 2) % 4
        peerPosition = gameState.getAgentPosition(peerIndex)
        self.status = self.offense
        if (not gameState.getAgentState(self.index).isPacman) and (gameState.getAgentState(self.index).scaredTimer == 0):
                ...
                if myState.isPacman and opponentState.isPacman:
                    ...
                    if opponentState.scaredTimer == 0:
                        ...
                        if self.getMazeDistance(myPosition, opponentPosition) <= 3:
                            ...
                            self.status = self.offense
                            self.save = True
                            reward[self.nearestPosition(self.getFood(gameState).asList())] = 1
                        else:
                            self.status = self.offense
                    ...
                    if myState.scaredTimer == 0:
                        ...
                        if self.getMazeDistance(myPosition, opponentPosition) <= 5:
                            ...
                            bestPosition = self.bestDefensePosition(opponentGameState, opponent, myPosition, opponentGameState.getInitialAgentPosition(opponent))
                    ...
                    if myState.scaredTimer > 0:
                        ...
                        if self.getMazeDistance(myPosition, opponentPosition) <= 2:
                            reward[opponentPosition] = -self.totalFood - 10
                            if len(self.goodFood) > 0:
                                self.food = random.choice(self.goodFood)
                            else:
    ...
    def bestDefensePosition(self, gameState, opponent, myPosition, initialPosition):
        mindis = 0
        bestAction = "Stop"
        for action in gameState.getLegalActions(self.index):
            currentState = gameState.deepCopy()
            self.setWalls(currentState, myPosition, 2)

            if len(currentState.getLegalActions(opponent)) == 0 or self.getMazeDistance(myPosition, gameState.getAgentPosition(opponent)) == 1:
                return gameState.getAgentPosition(opponent)
            state = gameState.deepCopy()
            state = state.generateSuccessor(self.index, action)
            self.setWalls(state, state.getAgentPosition(self.index), 2)
            dis = self.getBlockedDistance(state, state.getAgentPosition(opponent), state.getInitialAgentPosition(opponent)) - self.getMazeDistance(state.getAgentPosition(self.index), state.getAgentPosition(opponent))
            if dis > mindis:
                mindis = dis
                bestAction = action

        bestPosition = gameState.generateSuccessor(self.index, bestAction).getAgentPosition(self.index)
        return bestPosition
    ...
    def nearestFood(self, foodList):
        ...
        if self.food not in foodList:
            self.food = random.choice(foodList)
    ...
    def distanceMazeToCapsule(self,gameState, position, capsuleList):
        mindis = 10000
        for capsule in capsuleList:
            d = self.getMazeDistance(position, capsule)
            if d < mindis:
                mindis = d
...
class Node():
    ...
    def heuristicToReward(self):
        ...
        if self.captureAgent.status == self.captureAgent.escape:
            ...
            if len(chosenList) > 0 and (not self.captureAgent.start in self.rewardList.keys()) or ((len(chosenList) > 0 and (self.captureAgent.save == True or self.captureAgent.getBlockedDistance(self.state, self.position, self.captureAgent.start) > 5000))):
                ...
                h = math.pow(0.9,(self.captureAgent.getBlockedDistance(self.state, self.position, self.captureAgent.start) + self.captureAgent.getMazeDistance(self.position, self.captureAgent.start) * 0.001))
            ...
            if self.captureAgent.save:
                if len(self.captureAgent.getCapsules(self.state)) > 0:
                    ...
                    #     print(peerDis)
                        print("save")
                        h = -100 - 100 * len(self.captureAgent.getCapsules(self.state)) - self.captureAgent.distanceToCapsule(self.state, self.position, self.captureAgent.getCapsules(self.state))
                        # print(self.captureAgent.distanceToCapsule(self.state, self.position, self.captureAgent.getCapsules(self.state)))
                        return h
            for rewardPosition in self.rewardList.keys():
    ...
    def selectValueChild(self):
        values = [(child.heuristicToReward() + self.nways * 0.0000001) for child in self.children]
        # for child in self.children:
        #    print(child.value)
        for child in self.children:
        ...
        if len(values) == 0:
            return None
        maxValue = max(values)
        bestChilds = [child for child, v in zip(self.children, values) if v == maxValue]

        if self.captureAgent.status == self.captureAgent.escape:
            for child in bestChilds:
                if self.getChildAction(child) == "West":
                    return child
                if self.getChildAction(child) == "East":
                    return child
        bestChild = random.choice(bestChilds)
        return bestChild