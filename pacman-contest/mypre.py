...
class myAgent(CaptureAgent):
    ...
    def chooseAction(self, gameState):
        ...
        while self.getMazeDistance(self.start,self.targetFood) > 10000:
            self.targetFood = random.choice(self.getFood(gameState).asList())
        time1 = time.time()
    ...
    def getFoodDistance(self, gameState):
        ...
        # if len(foodList) != self.totalFoodNumber and not self.getFood(gameState).asList() == []:
    ...
    def getFeatures(self,gameState, action):
        ...
        if self.cost < 5 or self.foodLeft <= 2 or self.getTeamateDistance(successor) > 6:
            features["friend"] = 0
        if features["capsule"] < -1 or features["capsule"] == 0.9:
            features["capsule"] = 10
        if self.getEnermy(gameState)[0][2] < 8 and self.getEnermy(gameState)[0][1] is not None \
            ...
            if features["ghostMin"] > 1:
                features["ghostMin"] = -1
            print("chase1", action, features)
            return util.Counter(features)
        #chase pacman
        if self.getEnermy(gameState)[1][2] < 8 and self.getEnermy(gameState)[1][1] is not None \
    ...
    def goEat(self,gameState, successor):
        print("food distance",self.getMazeDistance(gameState.getAgentPosition(self.index), self.targetFood))
    ...
    def goBack(self,gameState, successor):
        if self.foodLeft <= 2:
            return True
        if (gameState.getAgentState(self.index).numCarrying > 11 and self.getMazeDistance(
                gameState.getAgentPosition(self.index), self.targetFood) > 11):
            return True
    ...
    def setup(self,actions=["North", "South", "West", "East","Stop"], learning_rate=0.01, reward_decay=0.9, e_greedy=1,
        ...
        if first:
            self.weights = util.Counter({"food": 93.19970348920681,
                                         ...
                                         "distanceHome": 1138.773437266105,
                                         "capsule":  90,
                                         "ghostMin": -100,
                                         "ghostMax": -100,
                                         "ways": 0,
