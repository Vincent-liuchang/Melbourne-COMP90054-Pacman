...
class SmartOffense(CaptureAgent):
	...
	def registerInitialState(self, gameState):
		...
		self.dangeFood = []
		self.mode = 1
		self.weights1 = {'closest-food': 2.35219445732408, 'bias': 2.579502234147277, 'closest-capsule': 2.3714473123398,"team":-0.05}
	...
	def chooseAction(self,gameState):
		...
		if self.mode ==1:
			...
			# print("mode22222222")
			print(self.chooseDeAction(gameState))
	...
	def getFirstFeatures(self, gameState, action):
		...
		minScare = min(scared)
		# print(self.whetherClosestCapsule(gameState))
		if minScare<5:
			eat = 1
		else:
			eat = -1
	...
	def getEntry(self,gameState):
		...
		for each in available:
			if self.red:
				...
				print(self.getOpenEntry(each,"left",gameState,available))
