...
class myAgent(CaptureAgent):
    ...
    def changeFartestTarget(self, gameState):
        foodList = self.getFood(gameState).asList()
    ...
    def chooseAction(self, gameState):
        ...
        if self.getPreviousObservation() is not None:
            if self.targetFood not in self.food or self.getPreviousObservation().getAgentState(
                ...
                if self.targetFood[0] == gameState.getAgentPosition((self.index + 2) % 4)[0] and self.targetFood[1] == \
                        gameState.getAgentPosition((self.index + 2) % 4)[1]:
                    self.setUpTarget(gameState)
                else:
        ...
        if self.targetFood[0] == targetFood[0] and self.targetFood[1] == targetFood[1] \
            ...
            self.changeFartestTarget(gameState)
    ...
    def getCapsuleDistance(self, gameState):
        ...
        if ca_list != []:
            ...
            for x, y in ca_list:
                ca = tuple((x, y))
                if my_position[0] == self.myPosition[0] and my_position[1] == self.myPosition[1]:
                    if ca in self.myDistance:
                        ca_distance.append(self.myDistance[ca])
                    else:
    ...
    def getFeatures(self, gameState, action):
        ...
        if (self.getEnermy(gameState)[0][0] and self.getEnermy(gameState)[0][1] is not None
                ...
                and gameState.getAgentState(self.index).scaredTimer == 0 and (not self.getEnermy(gameState)[0][3]))\
        ...
        if (self.getEnermyDistanceToMe(gameState)[0] < 4 and gameState.getAgentState(self.index).isPacman and not self.getEnermy(gameState)[0][3])\
                ...
                or (self.computeDistances(gameState)[self.start]>10000 and min(self.getCapsuleDistance(gameState)) > 10000):
            if len(self.food) != 0:
        ...
        if self.goBackScore(gameState, successor) or successor.data._win:
            features["foodCarry"] = abs(successor.getAgentState(self.index).numCarrying-gameState.getAgentState(self.index).numCarrying) * 30
            features["distanceHome"] = self.getHomeDistance(gameState) - self.getHomeDistance(successor)
    ...
    def goBack(self,gameState, successor):
        ...
        if gameState.getAgentState(self.index).numCarrying >= 1 and self.cost >= 280:
            return True
        if self.targetFood in self.myDistance and self.myDistance[self.targetFood] > 1000 and min(self.getAllFoodDistance(gameState)) < 10000 :
            if len(self.food) != 0:
                self.targetFood = random.choice(self.getFood(gameState).asList())
