...
class MonteCarloOffensiveAgent(CaptureAgent):
    ...
    def chooseAction(self, gameState):
        ...
        if foodLeft <= 2 or gameState.data.timeleft < 100:
            self.escape = True
            self.reward[self.start] = 100000000
        ...
        if gameState.getAgentState(self.index).scaredTimer == 0 and self.defense:
        ...
        for opponent in self.getOpponents(gameState):
            if gameState.getAgentPosition(opponent) != None:
                if (gameState.getAgentState(opponent).isPacman & (
                    ...
                    gameState.getAgentPosition(self.index)) <= 4 and (not gameState.getAgentState(
                ...
                if (gameState.getAgentState(opponent).scaredTimer > 5 & gameState.getAgentState(self.index).isPacman):
                    ...
                    if self.getMazeDistance(gameState.getAgentPosition(opponent),
                                            gameState.getAgentPosition(self.index)) <= 4:
                        if not gameState.getAgentState(self.index).isPacman:
                            self.defense = True

                        reward[
                            gameState.getAgentPosition(opponent)] = -self.totalFood - 10 + self.getFoodYouAreDefending(
                            gameState).asList().__len__()
...
class Node():
    ...
    def heuristicToReward(self):
        ...
        if self.captureAgent.escape:
            if self.captureAgent.start in self.rewardList:
            ...
            if len(self.captureAgent.getCapsules(self.state)) > 0:
                for capsule in self.captureAgent.getCapsules(self.state):
                    h += -self.captureAgent.getMazeDistance(capsule, self.state.getAgentPosition(self.index))
                for o in opponents:
